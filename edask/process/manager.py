...
class ExecResultHandler(ResultHandler):
    ...
    def _processFinalResult( self ):
        ...
        savePath = result.save(self.archivePath( result["archive"], result.product ) )
        if self.portal:
    ...
    def archivePath(self, archiveSpec: str, type: str = None  ) -> str:
        if not archiveSpec: return self.filePath
        toks = archiveSpec.split("/")
        proj =  toks[0]
        exp = toks[1] if len(toks) > 1 else Job.randomStr(6)
        atype = type if type else toks[2] if len(toks) > 2 else "analytics"
        return Archive.getFilePath( proj, exp, atype )
