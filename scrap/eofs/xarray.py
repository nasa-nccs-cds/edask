...
from .tools.xarray import (find_time_coordinates, categorise_ndcoords, weights_array)


class Eof(object):
    """EOF analysis (meta-data enabled `xarray` interface)"""

    def __init__(self, array, weights=None, center=True, scale=True, ddof=1):
    ...
    def eofs(self, eofscaling=0, neofs=None):
        ...
        **Examples:**
        ...
        eofdim = xr.IndexVariable('mode', range(eofs.shape[0]), attrs={'long_name': 'eof_mode_number'})
        coords = [eofdim] + self._coords
        long_name = 'empirical_orthogonal_functions'
        eofs = xr.DataArray(eofs, coords=coords, name='eofs',  attrs={'long_name': long_name})
        eofs.coords.update( { coord.name: (coord.dims, coord) for coord in self._space_ndcoords } )
        return eofs
